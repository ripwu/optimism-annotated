...
import { getDeployConfig, getContractFromArtifact } from '../src/deploy-utils'
...
task('take-dump').setAction(async (args, hre) => {
  ...
  const variables = {
    ...
    OVM_GasPriceOracle: {
      ...
      gasPrice: deployConfig.gasPriceOracleL2GasPrice || 1,
      l1BaseFee: deployConfig.gasPriceOracleL1BaseFee || 1,
      overhead: deployConfig.gasPriceOracleOverhead || 2750,
      scalar: deployConfig.gasPriceOracleScalar || 1_500_000,
      decimals: deployConfig.gasPriceOracleDecimals || 6,
    },
    L2StandardBridge: {
      l1TokenBridge: (
  ...
  const genesis = {
    ...
    config: {
      ...
      istanbulBlock: 0,
      muirGlacierBlock: 0,
      berlinBlock: deployConfig.hfBerlinBlock || 0,
      clique: {
        period: 0,
        epoch: 30000,
      },
